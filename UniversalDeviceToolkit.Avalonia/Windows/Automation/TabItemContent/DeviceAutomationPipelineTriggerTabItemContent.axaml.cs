...
{
public partial class DeviceAutomationPipelineTriggerTabItemContent : global::Avalonia.Controls.UserControl, IAutomationPipelineTriggerTabItemContent<INativeWindowsMessagePipelineTrigger>
{
    ...
    private async void FilterTextBox_TextChanged(object sender, TextChangedEventArgs e)
    {
        try
        {
            ...
            try { oldCts?.Dispose(); } catch (Exception ex) { if (Log.Instance.IsTraceEnabled) Log.Instance.Trace($"Failed to dispose old CancellationTokenSource in {nameof(FilterTextBox_TextChanged)}.", ex); }
