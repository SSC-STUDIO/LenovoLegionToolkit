...
public partial class NavigationItemsSettingsWindow : BaseWindow
{
    ...
    private async void NavigationItemsSettingsWindow_Loaded(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            _keyboardCard.IsVisible = keyboardSupported ? true : false;
    ...
    private void NavigationItemToggle_Click(object? sender, RoutedEventArgs e)
    {
        ...
        if (sender is not ToggleSwitch toggleSwitch || toggleSwitch.Tag is not string pageTag)
            return;
        ...
        if (UdtAppContext.MainWindow is MainWindow mainWindow)
        {
