...
public partial class OsdPanelWindow : global::UniversalDeviceToolkit.Avalonia.Windows.Osd.OsdWindowBase
{
    ...
    protected override void ApplyAppearanceSettings()
    {
        ...
        try
        {
            var color = Color.Parse(_OsdSettings.Store.BackgroundColor);
        ...
        {
            // AVALONIA: Style has no BasedOn; inherit the original style's setters instead.
            var newStyle = new Style(x => x.OfType<TextBlock>());
            foreach (var setter in _originalLabelStyle.Setters)
        ...
        {
            var newStyle = new Style(x => x.OfType<TextBlock>());
            foreach (var setter in _originalValueStyle.Setters)
                newStyle.Setters.Add(setter);
            newStyle.Setters.Add(new Setter(TextBlock.FontSizeProperty, fontSize + 1));
    ...
    protected override void SetDefaultWindowPosition()
    {
        ...
        var screen = this.Screens.Primary ?? this.Screens.ScreenFromWindow(this);
        ...
        Position = new PixelPoint(
            (int)workArea.Left,
            (int)(workArea.Top + (workArea.Height - actualHeight) / 2));
        _positionSet = true;
    }

    protected override void OnItemVisibilityChanged(Control element, bool visible)
    {
        var visibility = visible ? true : false;

        if (element.Parent is not Panel panel) return;

        foreach (var child in System.Linq.Enumerable.OfType<TextBlock>(panel.Children))
        {
            if (child != element) child.IsVisible = visibility;
        }
    }
