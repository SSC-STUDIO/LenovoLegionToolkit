...
using UniversalDeviceToolkit.Avalonia.Windows.Automation;
...
{
public partial class AutomationPage : global::Avalonia.Controls.UserControl
{
    ...
    {
        var existingTriggersTypes = System.Linq.Enumerable.ToArray(_automaticPipelinesStackPanel.Children)
        ...
        var window = new CreateAutomationPipelineWindow(existingTriggersTypes, AddAutomaticPipeline);
    ...
    private async void SaveButton_Click(object? sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var automaticPipelines = System.Linq.Enumerable.ToArray(_automaticPipelinesStackPanel.Children)
                .OfType<AutomationPipelineControl>()
                .Select(c => c.CreateAutomationPipeline())
                .ToList();

            var manualPipelines = System.Linq.Enumerable.ToArray(_manualPipelinesStackPanel.Children)
                .OfType<AutomationPipelineControl>()
                .Select(c => c.CreateAutomationPipeline())
                .ToList();
    ...
    private async Task RefreshAsync()
    {
        ...
        _saveRevertStackPanel.IsVisible = false;

        _noAutomaticActionsText.IsVisible = _automaticPipelinesStackPanel.Children.Count < 1;
        _noManualActionsText.IsVisible = _manualPipelinesStackPanel.Children.Count < 1;
    ...
    private AutomationPipelineControl GenerateControl(AutomationPipeline pipeline, Panel stackPanel, bool allowQuickActionAutomationStep = true)
    {
        ...
        control.PointerReleased += PipelineControl_PointerReleased;
        control.OnChanged += PipelineControl_OnChanged;
        control.OnDelete += PipelineControl_OnDelete;
        return control;
    }

    private void PipelineControl_PointerReleased(object? sender, PointerReleasedEventArgs e)
    {
    ...
    {
        _saveRevertStackPanel.IsVisible = true;
    }
    ...
    private void MovePipeline(Control control, Panel stackPanel, int index)
    {
    ...
    private async Task ChangePipelineIconAsync(AutomationPipelineControl control)
    {
        try
        {
            var window = new SymbolRegularPicker();
            window.ShowDialog(UdtAppContext.MainWindow);
    ...
    private void DeletePipeline(Control control, Panel stackPanel)
    {
