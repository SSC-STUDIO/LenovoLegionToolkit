...
using UniversalDeviceToolkit.Avalonia.Controls.Charts;
using UniversalDeviceToolkit.Avalonia.Utils;
using UniversalDeviceToolkit.Avalonia.Controls;
using CustomControls = UniversalDeviceToolkit.Avalonia.Controls.Custom;

namespace UniversalDeviceToolkit.Avalonia.Pages.WindowsOptimization;

public partial class NetworkAccelerationControl : global::Avalonia.Controls.UserControl
{
    ...
    public NetworkAccelerationControl()
    {
        ...
        PropertyChanged += NetworkAccelerationControl_IsVisibleChanged;
    }
    ...
    private void NetworkAccelerationControl_IsVisibleChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
    ...
    private void NatStatus_Click(object? sender, RoutedEventArgs e) =>
        ShowDiagnosticPopup(_natDetailsPopup, sender as Control);

    private void DnsStatus_Click(object? sender, RoutedEventArgs e) =>
        ShowDiagnosticPopup(_dnsDetailsPopup, sender as Control);

    private void Ipv6Status_Click(object? sender, RoutedEventArgs e) =>
        ShowDiagnosticPopup(_ipv6DetailsPopup, sender as Control);

    private void ShowDiagnosticPopup(Popup popup, Control? placementTarget)
    {
        CloseDiagnosticPopups();
    ...
    {
        return (this.TryFindResource(resourceKey) as SolidColorBrush)?.Color ?? fallback;
    }
    ...
    private Control CreateConnectionRow(NetworkProxyConnectionSnapshot connection)
    {
        ...
        var title = new TextBlock
        {
            ...
            FontSize = (double)this.TryFindResource("FontSizeCaption")!,
            Foreground = (Brush)this.TryFindResource("TextFillColorPrimaryBrush")!,
        ...
        var detail = new TextBlock
        {
            ...
            Foreground = state is "failed" or "blocked"
                ? (Brush)this.TryFindResource("StatusCriticalBrush")!
                : (Brush)this.TryFindResource("TextFillColorSecondaryBrush")!,
            VerticalAlignment = VerticalAlignment.Center
        };
    ...
    private Control CreateDestinationRow(NetworkProxyDestinationSnapshot destination)
    {
        ...
        var detail = new TextBlock
        {
            Text = string.Format(
                ...
                latency),
            FontSize = (double)this.TryFindResource("FontSizeCaption")!,
            Foreground = (Brush)this.TryFindResource("TextFillColorSecondaryBrush")!,
            VerticalAlignment = VerticalAlignment.Center
        };
        Grid.SetColumn(detail, 1);
        row.Children.Add(title);
    ...
    private void InitDiagnosticsCombos()
    {
        if (_natStunCombo is not null)
        {
            ...
            _natStunCombo.SelectedItem = "stun.miwifi.com";
        }
        ...
        if (_dnsServerCombo is not null)
        {
            ...
            _dnsServerCombo.SelectedItem = "223.5.5.5";
        }
        ...
        if (_dnsDohUrlCombo is not null)
        {
            ...
            _dnsDohUrlCombo.SelectedItem = "https://doh.pub/dns-query";
        }
    ...
    private async void NatDetectButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var stunHost = (_natStunCombo?.SelectedItem as string ?? "stun.miwifi.com").Trim();
    ...
    private async void DnsDetectButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var dnsServer = (_dnsServerCombo?.SelectedItem as string ?? string.Empty).Trim();
            var dohEnabled = _dnsDohToggle?.IsChecked == true;
            var dohUrl = (_dnsDohUrlCombo?.SelectedItem as string ?? string.Empty).Trim();
        ...
        try
        {
            ...
            if (_ipv6SupportText is not null)
            {
                ...
                _ipv6SupportText.Foreground = result.Supported
                    ? (this.TryFindResource("StatusSuccessBrush") as Brush ?? new SolidColorBrush(Colors.Green))
                    : (this.TryFindResource("TextFillColorPrimaryBrush") as Brush ?? new SolidColorBrush(Colors.Black));
            }
    ...
    private void BuildServiceList()
    {
        ...
        if (groups is null || groups.Count == 0)
        {
            if (_domainGroupsEmptyState is not null)
                _domainGroupsEmptyState.IsVisible = true;
            return;
        }

        if (_domainGroupsEmptyState is not null)
            _domainGroupsEmptyState.IsVisible = false;
    ...
    private CustomControls.CardExpander CreateServiceGroupRow(NetworkDomainGroup group)
    {
        ...
        nameText.Styles.Add((Style)this.TryFindResource("NaServiceNameStyle")!);
        var favStar = new TextBlock
        {
            ...
            VerticalAlignment = VerticalAlignment.Center,
            IsVisible = group.IsFavorite ? true : false,
            Foreground = (Brush)this.TryFindResource("PaletteOrangeBrush")!
        };
        ...
        expander.Styles.Add((Style)this.TryFindResource("NaServiceExpanderStyle")!);
    ...
    private Grid CreateSubItemRow(NetworkDomainGroup group, NetworkDomainSubItem sub)
    {
        ...
        nameText.Styles.Add((Style)this.TryFindResource("NaSubItemNameStyle")!);

        // Beta tag
        Control? betaTag = null;
        if (sub.IsBeta)
        {
            var betaBorder = new Border
            {
                Child = new TextBlock
                {
                    Text = "Beta",
                    FontSize = 10,
                    Foreground = (Brush)this.TryFindResource("TextOnAccentFillColorPrimaryBrush")!
                }
            };
            betaBorder.Styles.Add((Style)this.TryFindResource("NaBetaTagStyle")!);
            betaTag = betaBorder;
        }
        ...
        domainText.Styles.Add((Style)this.TryFindResource("NaSubItemDomainStyle")!);
        ...
        wrapper.Styles.Add((Style)this.TryFindResource("NaSubItemRowStyle")!);
    ...
    private Border CreateBrandIcon(string? iconKey)
    {
        var background = iconKey switch
        {
            ...
            _ => (Brush)(this.TryFindResource("ControlFillColorSecondaryBrush") ?? Brushes.Transparent)
        };

        var foreground = iconKey is "RobloxLogo" or "CdnLogo"
            ? (IBrush)new SolidColorBrush(Color.FromRgb(101, 106, 113))
            : Brushes.White;
        ...
        if (iconKey is not null && BrandIconGeometry.TryGetValue(iconKey, out var geometryData))
        {
            ...
            iconBorder.Styles.Add((Style)this.TryFindResource("NaBrandIconStyle")!);
            return iconBorder;
        }
        ...
        fallbackBorder.Styles.Add((Style)this.TryFindResource("NaBrandIconStyle")!);
    ...
    private void RefreshUi()
    {
        ...
        try
        {
            ...
            if (_modeComboBox is not null)
            {
                ...
                foreach (var item in _modeComboBox.Items.OfType<ModeOption>())
                {
            ...
            if (_runStateDot is not null)
                _runStateDot.Fill = (Brush)this.TryFindResource(isRunning ? "StatusSuccessBrush" : "TextFillColorTertiaryBrush")!;
            if (_runErrorText is not null)
            {
                ...
                _runErrorText.Text = !isRunning && !canStart
                    ? reason
                    : string.Empty;
                _runErrorText.IsVisible = string.IsNullOrWhiteSpace(_runErrorText.Text)
                    ? false
                    : true;
            }
    ...
    private void UpdateTrafficSectionVisibility(bool? isRunning = null)
    {
        ...
        _trafficSection.IsVisible = (isRunning ?? _acceleration.IsRunning)
            ? true
            : false;
    }
