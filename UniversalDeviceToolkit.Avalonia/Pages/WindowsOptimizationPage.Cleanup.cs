...
public partial class WindowsOptimizationPage
{
    ...
    private async void RunCleanupButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(() => SnackbarHelper.Show(Resource.SettingsPage_WindowsOptimization_Title, summary, severity));
        }
        catch (Exception ex)
        {
            if (Log.Instance.IsTraceEnabled)
            ...
            await Dispatcher.UIThread.InvokeAsync(() => SnackbarHelper.Show(Resource.SettingsPage_WindowsOptimization_Title, Resource.SettingsPage_WindowsOptimization_Cleanup_Error, SnackbarType.Error));
        }
        finally
        {
    ...
    {
        if (sender is global::Avalonia.Controls.Control element && element.Tag is CustomCleanupRuleViewModel rule)
        {
            using var dialog = new System.Windows.Forms.FolderBrowserDialog();
