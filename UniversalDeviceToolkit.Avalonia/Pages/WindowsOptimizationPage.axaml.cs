...
using WpfMenuItem = UniversalDeviceToolkit.Avalonia.Controls.MenuItem;

namespace UniversalDeviceToolkit.Avalonia.Pages
{
public partial class WindowsOptimizationPage : global::Avalonia.Controls.UserControl
{
    ...
    private void UpdateNetworkAccelerationSelectionChrome()
    {
        ...
        ToolTip.SetTip(_networkAccelerationSelectionStartButton, _networkAccelerationSelectionStartButton.IsEnabled
                ? actionLabel
    ...
    private void NetworkAccelerationSelectionFavoriteButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        foreach (var group in _networkAccelerationControl.GetRecommendedTargetGroups())
        {
            ...
            // AVALONIA: StaysOpenOnClick removed — clicking a checkable item closes the menu.
        ...
        if (_networkAccelerationRecommendationsMenu is not null)
            _networkAccelerationRecommendationsMenu.Close();
    ...
    private static IEnumerable<T> EnumerateVisualDescendants<T>(Visual root) where T : Visual
    {
    ...
    private void ScrollViewer_PointerWheelChanged(object? sender, PointerWheelEventArgs e)
    {
        ...
        {
            e.Handled = true;
            var offset = scrollViewer.Offset.Y - (e.Delta.Y / 3.0);
            var maxOffset = Math.Max(0, scrollViewer.Extent.Height - scrollViewer.Viewport.Height);
            scrollViewer.Offset = scrollViewer.Offset.WithY(Math.Max(0, Math.Min(offset, maxOffset)));
        }
    }
    ...
    private void OpenStyleSettingsButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (owner is not null)
                pluginSettingsWindow.ShowDialog(owner);
        }
        catch (Exception ex)
        {
            if (Log.Instance.IsTraceEnabled)
    ...
    private void ActionItem_PointerPressed(object? sender, PointerPressedEventArgs e)
    {
        if (sender is not Control element || element.DataContext is not OptimizationActionViewModel actionVm)
            return;
