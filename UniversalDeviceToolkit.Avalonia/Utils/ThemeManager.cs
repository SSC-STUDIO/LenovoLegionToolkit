...
public class ThemeManager
{
    ...
    public ThemeManager(SystemThemeListener systemThemeListener, ApplicationSettings settings)
    {
        ...
        _listener.Changed += (_, _) => Dispatcher.UIThread.Post(Apply);

        // IoC AutoActivate resolves this before any window is shown; apply saved theme immediately
        // so App.axaml's hard-coded Dark defaults do not flash on light/system mode.
        Apply();
    }
    ...
    public void Apply()
    {
        ...
        AppScaleManager.ApplySaved(_settings);
    ...
    private void SetTheme()
    {
        ...
        // AVALONIA: Wpf.Ui ApplicationThemeManager.Apply(theme, backgroundType, false) is
        // replaced by Avalonia's RequestedThemeVariant; backdrops are applied per window
        // via TransparencyLevelHint (see UpdateWindowBackdrops).
    ...
    private void UpdateWindowBackdrops()
    {
        ...
        // AVALONIA: Application.Current.Windows does not exist; windows are enumerated
        // through the desktop application lifetime.
        if (Application.Current?.ApplicationLifetime is not IClassicDesktopStyleApplicationLifetime { Windows: { } windows })
            return;
        ...
        foreach (Window window in windows)
        {
            ...
            {
                baseWindow.TransparencyLevelHint = new[] { backgroundType };
    ...
    private void SetColor(RGBColor accent)
    {
        ...
        // AVALONIA: Wpf.Ui ApplicationAccentColorManager.Apply is replaced by FluentTheme's
        // SystemAccentColor* color resources plus the Wpf.Ui-style accent brush keys the
        // app XAML references (AccentFillColor*/TextOnAccent*/AccentTextFillColor*).
        Application.Current.Resources["SystemAccentColor"] = accentColor;
        Application.Current.Resources["SystemAccentColorLight1"] = BlendToward(accentColor, Colors.White, 0.35);
        Application.Current.Resources["SystemAccentColorLight2"] = BlendToward(accentColor, Colors.White, 0.65);
        Application.Current.Resources["SystemAccentColorLight3"] = BlendToward(accentColor, Colors.White, 0.85);
        Application.Current.Resources["SystemAccentColorDark1"] = BlendToward(accentColor, Colors.Black, 0.35);
        Application.Current.Resources["SystemAccentColorDark2"] = BlendToward(accentColor, Colors.Black, 0.65);
        Application.Current.Resources["SystemAccentColorDark3"] = BlendToward(accentColor, Colors.Black, 0.85);

        SetBrush("SystemAccentColorBrush", accentColor);
        SetBrush("SystemAccentColorPrimaryBrush", accentColor);
        SetBrush("AccentFillColorDefaultBrush", accentColor);
        SetBrush("AccentFillColorSecondaryBrush", BlendToward(accentColor, Colors.White, 0.18));
        SetBrush("AccentFillColorTertiaryBrush", BlendToward(accentColor, Colors.White, 0.32));

        var textOnAccent = IsLightColor(accentColor) ? Colors.Black : Colors.White;
        Application.Current.Resources["TextOnAccentFillColorPrimary"] = textOnAccent;
        SetBrush("TextOnAccentFillColorPrimaryBrush", textOnAccent);
        SetBrush("TextOnAccentFillColorSecondaryBrush", textOnAccent);
        SetBrush("TextOnAccentFillColorTertiaryBrush", textOnAccent);

        SetBrush("AccentTextFillColorPrimaryBrush", accentColor);
        SetBrush("AccentTextFillColorSecondaryBrush", accentColor);
        SetBrush("AccentTextFillColorTertiaryBrush", accentColor);
    }
