...
public sealed class StartupDeviceSetupCoordinator
{
    ...
    public async Task RunIfNeededAsync(MachineInformation machineInformation)
    {
        ...
        {
            // InvokeAsync wraps the inner task; await it so setup actually
            // completes on the UI thread.
            var uiTask = Dispatcher.UIThread.InvokeAsync(() => RunIfNeededAsync(machineInformation));
        ...
        var mainWindow = UdtAppContext.MainWindow;
        ...
        try
        {
            // AVALONIA: Avalonia cannot assign Window.Owner directly; Show(owner) attaches
            // the owner and disables it for the lifetime of the shown window.
