...
internal static class NavigationPaneMetrics
{
    ...
    {
        if (Application.Current?.TryFindResource("NavigationWidthCollapsed", out var collapsedValue) == true
            && collapsedValue is double width && width > 0)
            return width;
        return 70;
    }
    ...
    {
        if (Application.Current?.TryFindResource("NavigationWidthExpanded", out var expandedValue) == true
            && expandedValue is double width && width > 0)
            return width;
        return 220;
    }
