...
using UniversalDeviceToolkit.Avalonia.Controls.Automation.Steps;
...
public class AutomationPipelineControl : UserControl
{
    ...
    public AutomationPipelineControl(AutomationPipeline automationPipeline, IAutomationStep[] supportedAutomationSteps)
    {
        ...
        ToolTip.SetTip(_isExclusiveCheckBox, Resource.AutomationPipelineControl_Exclusive_ToolTip);

        Initialized += AutomationPipelineControl_Initialized;
        Unloaded += AutomationPipelineControl_Unloaded;
    }

    private void AutomationPipelineControl_Unloaded(object? sender, RoutedEventArgs e)
    {
        Unloaded -= AutomationPipelineControl_Unloaded;
        foreach (var step in _stepsStackPanel.Children.OfType<AbstractAutomationStepControl>().ToList())
        {
            step.ContextMenu = null;
        }
        _stepsStackPanel.Children.Clear();
    ...
    public AutomationPipeline CreateAutomationPipeline() => new()
    {
        ...
        Steps = _stepsStackPanel.Children
    ...
    private async void AutomationPipelineControl_Initialized(object? sender, EventArgs e)
    {
        try
        {
            ...
            {
                _isExclusiveCheckBox.IsVisible = false;
            }
            ...
            _addStepButton.Click += async (_, _) =>
            {
                try
                {
                    ...
                    var window = new AddAutomationStepWindow(stepControls, AddStep);
    ...
    {
        var stepsCount = _stepsStackPanel.Children
            .OfType<AbstractAutomationStepControl>()
            .Count();
    ...
    private async Task<AbstractAutomationStepControl> GenerateStepControlAsync(IAutomationStep automationStep)
    {
        AbstractAutomationStepControl control = automationStep switch
        {
            ...
            ShowMainWindowAutomationStep s => new ShowMainWindowAutomationStepControl(s),
            HideMainWindowAutomationStep s => new HideMainWindowAutomationStepControl(s),
