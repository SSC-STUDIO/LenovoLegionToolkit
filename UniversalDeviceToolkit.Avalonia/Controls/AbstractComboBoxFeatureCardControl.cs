...
public abstract class AbstractComboBoxFeatureCardControl<T> : AbstractRefreshingControl where T : struct
{
    ...
    protected virtual Control GetAccessory(ComboBox comboBox) => comboBox;
    ...
    {
        MessagingCenter.Subscribe<FeatureStateMessage<T>>(this, () => Dispatcher.UIThread.InvokeTask(async () =>
        {
            if (!IsVisible)
                return;
