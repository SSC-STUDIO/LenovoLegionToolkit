...
public partial class AppNotificationHost : global::Avalonia.Controls.UserControl
{
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        if (!_attached)
        {
            Dispatcher.UIThread.Post(TryAttachService, DispatcherPriority.ApplicationIdle);
        }
    }
    ...
    private void Service_Changed(object? sender, AppNotificationChangedEventArgs e)
    {
        ...
        {
            Dispatcher.UIThread.Post(() => Service_Changed(sender, e));
            return;
        }
    ...
    {
        if (sender is not Control { Tag: Guid id })
            return;
    ...
    private void Toast_MouseEnter(object sender, PointerEventArgs e)
    {
        if (sender is Control { DataContext: NotificationItemViewModel vm })
            vm.PauseTimer();
    }

    private void Toast_MouseLeave(object sender, PointerEventArgs e)
    {
        if (sender is Control { DataContext: NotificationItemViewModel vm })
            vm.ResumeTimer();
    }
