...
{
    private async void SensorsControl_IsVisibleChanged(object sender, AvaloniaPropertyChangedEventArgs e)
    {
            ...
            _refreshTask = Task.Run(async () =>
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        var detailed = await Dispatcher.UIThread.InvokeAsync<bool>(() =>
                            CanShowSensorDetails
                            && (_detailsExpanded
                                || _forceDetailedRefresh
                                || IsElementVisible("_cpuDetailsPanel")
                                || IsElementVisible("_gpuDetailsPanel")));
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (cached.HasValue)
                            await Dispatcher.UIThread.InvokeAsync(() => UpdateValues(cached.Value, recordTrendHistory: false));
                    }
