...
// and Avalonia/Lib UI services (SnackbarHelper, InstallerLaunchPathValidator,
...
public sealed class PackageControlViewModel : IProgress<float>, IDisposable
{
    ...
    public PackageControlViewModel(IPackageDownloader packageDownloader, Package package, Func<string> getDownloadPath)
    {
        ...
        // Controls are always constructed on the UI thread; capturing the UI dispatcher
    ...
    private async Task DownloadAndInstallPackageAsync()
    {
        ...
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
            ...
            RaiseSnackbar(Resource.PackageControl_Http404Error_Title, Resource.PackageControl_Http404Error_Message, SnackbarKind.Error);
            ResetPendingStatus();
        }
        catch (HttpRequestException ex)
        {
            ...
            RaiseSnackbar(Resource.PackageControl_HttpGeneralError_Title, Resource.PackageControl_HttpGeneralError_Message, SnackbarKind.Error);
            ResetPendingStatus();
        }
        catch (Exception ex)
        {
            ...
            RaiseSnackbar(Resource.PackageControl_GeneralError_Title, ex.Message, SnackbarKind.Error);
            ResetPendingStatus();
        }
        finally
        {
        ...
        if (result)
        {
            RaiseSnackbar(Resource.PackageControl_DownloadComplete_Title, string.Format(Resource.PackageControl_DownloadComplete_Message, Package.FileName), SnackbarKind.Success);
            CheckDownloadedFileState();
            ...
            else
            {
                ...
                else
                {
                    ...
                    RaiseSnackbar(Resource.PackageControl_InstallError_Title, Resource.PackageControl_InstallError_FileNotFound, SnackbarKind.Error);
                    ResetPendingStatus();
                }
            }
        }
    ...
    private async Task InstallPackageAsync()
    {
        ...
        try
        {
            ...
            if (!InstallerLaunchPathValidator.TryValidateForExecution(filePath, configuredDownloadPath, expectedInstallerFileName, out var safeInstallerPath, out var validationError))
            {
                ...
                RaiseSnackbar(Resource.PackageControl_InstallError_Title, validationError, SnackbarKind.Error);
                ResetPendingStatus();
                StatusRefreshRequested?.Invoke(this, EventArgs.Empty);
                return;
            }
            ...
            try
            {
                ...
                RaiseSnackbar(Resource.PackageControl_InstallStarted_Title, string.Format(Resource.PackageControl_InstallStarted_Message, Package.FileName), SnackbarKind.Success);
            ...
            catch (Exception ex)
            {
                ...
                RaiseSnackbar(Resource.PackageControl_InstallError_Title, ex.Message, SnackbarKind.Error);
                ResetPendingStatus();
            }
        }
        catch (Exception ex)
    ...
    private async Task HandleInstallProcessExitAsync(Process installProcess)
    {
        ...
        try
        {
            ...
            {
                RaiseSnackbar(Resource.PackageControl_InstallError_Title, failureMessage, SnackbarKind.Error);
            }
    ...
    public void Report(float value) => _dispatcher.Post(() =>
