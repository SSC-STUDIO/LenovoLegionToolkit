...
public sealed class TrendSeries
{
    ...
    public Pen GetOrCreateLinePen()
    {
        ...
        // AVALONIA: WPF StartLineCap/EndLineCap collapse into the single LineCap.
        var pen = new Pen(new SolidColorBrush(_cachedColor), 1.15, null, PenLineCap.Round, PenLineJoin.Round);
        _cachedLinePen = pen;
        return pen;
    }
}
...
public class TrendChartControl : Control
{
    ...
    static TrendChartControl()
    {
        AffectsRender<TrendChartControl>(
            ...
            ScaleLabelBrushProperty,
            PlotPaddingProperty);
    }

    public static readonly StyledProperty<int> CapacityProperty = AvaloniaProperty.Register<TrendChartControl, int>(
    ...
    public static readonly StyledProperty<Brush> GridlineBrushProperty = AvaloniaProperty.Register<TrendChartControl, Brush>(
        nameof(GridlineBrush), new SolidColorBrush(Colors.Transparent));
    ...
    public static readonly StyledProperty<Brush> BaselineBrushProperty = AvaloniaProperty.Register<TrendChartControl, Brush>(
        nameof(BaselineBrush), new SolidColorBrush(Colors.Transparent));
    ...
    public static readonly StyledProperty<Brush> ScaleLabelBrushProperty = AvaloniaProperty.Register<TrendChartControl, Brush>(
        nameof(ScaleLabelBrush), new SolidColorBrush(Colors.Gray));
    ...
    public static readonly StyledProperty<Thickness> PlotPaddingProperty = AvaloniaProperty.Register<TrendChartControl, Thickness>(
        nameof(PlotPadding), new Thickness(4));
    ...
    {
        var pad = PlotPadding;
        var width = Bounds.Width - pad.Left - pad.Right;
        var height = Bounds.Height - pad.Top - pad.Bottom;
        if (width <= 1 || height <= 1)
            return;

        var translate = new TranslateTransform(pad.Left, pad.Top);
        using (context.PushTransform(translate.Value))
        {
            DrawGridlines(context, width, height, GridlineBrush, BaselineBrush, ScaleLabelBrush);
    ...
    /// </summary>
    private static void DrawGridlines(DrawingContext context, double width, double height, Brush gridlineBrush, Brush baselineBrush, Brush scaleLabelBrush)
    {
        ...
        foreach (var (fraction, label) in scaleMarks)
        {
            ...
            context.DrawText(ft, new Point(3, y - ft.Height / 2.0));
        }
        ...
        bool drawLine)
    {
        ...
        using (var areaCtx = areaGeometry.Open())
        {
            ...
            areaCtx.BeginFigure(new Point(points[0].X, height), true);
            areaCtx.LineTo(points[0], true);
            ...
            for (var p = 0; p < points.Count - 1; p++)
            {
                ...
                lineCtx.CubicBezierTo(c1, c2, p2, true);
                areaCtx.CubicBezierTo(c1, c2, p2, true);
            }
        ...
        if (drawFill)
            context.DrawGeometry(series.GetOrCreateFill(), null, areaGeometry);
        if (drawLine)
            context.DrawGeometry(null, series.GetOrCreateLinePen(), lineGeometry);
    }
