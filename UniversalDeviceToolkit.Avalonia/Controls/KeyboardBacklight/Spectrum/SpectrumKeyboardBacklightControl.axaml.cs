...
using UniversalDeviceToolkit.Avalonia.Windows.KeyboardBacklight.Spectrum;
...
{
public partial class SpectrumKeyboardBacklightControl : global::UniversalDeviceToolkit.Avalonia.Controls.AbstractRefreshingControl, IDisposable
{
    ...
    public SpectrumKeyboardBacklightControl()
    {
        ...
        PropertyChanged += SpectrumKeyboardBacklightControl_IsVisibleChanged;
    ...
    {
        Dispatcher.UIThread.InvokeTask(async () =>
        {
            if (!IsVisible)
                return;
    ...
    private async void SpectrumKeyboardBacklightControl_IsVisibleChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Error in {nameof(SpectrumKeyboardBacklightControl_IsVisibleChanged)}: {ex.Message}", ex);
        }
    }

    private void SpectrumKeyboardBacklightControl_SizeChanged(object? sender, SizeChangedEventArgs e)
    {
        if (_device.RenderTransform is not ScaleTransform scaleTransform)
            return;

        var target = 0.75 * Bounds.Width / _device.Bounds.Width;
    ...
    private async void BrightnessSlider_OnValueChanged(object? sender, RangeBaseValueChangedEventArgs e)
    {
        try
        {
            ...
            {
                var value = await Dispatcher.UIThread.InvokeAsync(() => (int)_brightnessSlider.Value);
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Error in {nameof(BrightnessSlider_OnValueChanged)}: {ex.Message}", ex);
        }
    }
    ...
    private async void ProfileButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Error in {nameof(ProfileButton_OnClick)}: {ex.Message}", ex);
        }
    }
    ...
    private async void SwitchKeyboardLayout_Click(object? sender, RoutedEventArgs e)
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Error in {nameof(SwitchKeyboardLayout_Click)}: {ex.Message}", ex);
        }
    }
    ...
    private async void ResetToDefaultButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Error in {nameof(ResetToDefaultButton_Click)}: {ex.Message}", ex);
        }
    }
    ...
    protected override async Task OnRefreshAsync()
    {
        ...
        if (vantageStatus == SoftwareStatus.Enabled)
        {
            ...
            _noEffectsText.IsVisible = false;
            return;
        }
    ...
    private async Task StopAnimationAsync()
    {
        using (await _startStopAnimationLock.LockAsync())
        {
            ...
            var oldCts = _refreshStateCancellationTokenSource;
            _refreshStateCancellationTokenSource = new();
            try { oldCts?.Dispose(); } catch (Exception ex) { if (Log.Instance.IsTraceEnabled) Log.Instance.Trace($"Failed to dispose old CancellationTokenSource in {nameof(StopAnimationAsync)}.", ex); }
    ...
    private async Task RefreshProfileDescriptionAsync()
    {
        ...
        _noEffectsText.IsVisible = effects.IsEmpty() ? true : false;
    }
    ...
    private async Task ReplaceEffectAsync(Control effectControl, SpectrumKeyboardBacklightEffect effect)
    {
        DeselectAllButtons();
    ...
    private async Task DeleteEffectAsync(Control effectControl)
    {
