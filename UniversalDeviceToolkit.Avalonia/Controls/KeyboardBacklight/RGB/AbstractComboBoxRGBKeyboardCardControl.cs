...
public abstract class AbstractComboBoxRGBKeyboardCardControl<T> : UserControl
{
    ...
    {
        // AVALONIA: WPF IsEnabledChanged event -> class handler on the IsEnabled property
        IsEnabledProperty.Changed.AddClassHandler<AbstractComboBoxRGBKeyboardCardControl<T>>(CardControl_IsEnabledChanged);
    ...
    private static void CardControl_IsEnabledChanged(AbstractComboBoxRGBKeyboardCardControl<T> control, AvaloniaPropertyChangedEventArgs e)
    {
        if (control.IsEnabled)
            return;

        control._comboBox.SelectionChanged -= control.ComboBox_SelectionChanged;
        control._comboBox.ClearItems();
        control._comboBox.SelectionChanged += control.ComboBox_SelectionChanged;
    }
