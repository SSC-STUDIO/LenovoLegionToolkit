...
namespace UniversalDeviceToolkit.Avalonia.Startup
{
    public class StartupOrchestrator
    {
        ...
        public StartupOrchestrator(App app, string[] args)
        {
        ...
        private void DetermineAndApplySafeStartMode()
        {
            ...
            else if (persistedShouldEnterSafeMode)
            {
                ...
                try
                {
                    ...
                    {
                        Dispatcher.UIThread.Post(PublishSafeStartBootNotification);
                    }
                    else
                    {
        ...
        private async Task CheckCompatibilityAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                await ShowErrorWindowBlockingAsync(errorWindow);
        ...
        /// <summary>
        /// AVALONIA: WPF's ownerless <c>Window.ShowDialog()</c> has no Avalonia equivalent —
        /// Avalonia requires a visible owner for modal dialogs. The MainWindow is created but
        /// not yet shown at this point, so block on a Closed-event task source instead
        /// (same pattern as <see cref="UniversalDeviceToolkit.Avalonia.Utils.NativeMessageBox"/>).
        /// </summary>
        private static async Task ShowErrorWindowBlockingAsync(Window errorWindow)
        {
            var owner = UdtAppContext.MainWindow;
            ...
            errorWindow.Closed += (_, _) => completion.TrySetResult();
            errorWindow.Show();
        ...
        {
            // AVALONIA: removed WinForms SetHighDpiMode — Avalonia configures DPI awareness
            // at platform startup; calling it here could throw once awareness is already set.
            // AVALONIA: removed RenderOptions.ProcessRenderMode — Avalonia has no per-process
            // render-mode switch; software rendering is configured at platform startup. The
            // compatibility decision is still evaluated to preserve the startup API shape.
            _ = RenderingCompatibilityHelper.GetPreferredRenderMode(_settings);
        ...
        private Task CreateMainWindowAsync()
        {
            ...
            UdtAppContext.SetMainWindow(mainWindow);
            PluginHostContext.SetCurrent(new MainAppPluginHostContext(() => UdtAppContext.MainWindow));
        ...
        private Task ShowMainWindowAsync()
        {
            ...
            {
                Dispatcher.UIThread.Post(ShowAction, DispatcherPriority.Loaded);
            }

            Dispatcher.UIThread.Post(App.CheckPendingCrashReports, DispatcherPriority.Background);
