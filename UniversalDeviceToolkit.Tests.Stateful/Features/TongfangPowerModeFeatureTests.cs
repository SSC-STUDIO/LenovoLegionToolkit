...
public class TongfangPowerModeFeatureTests
{
    ...
    private static FakeEcChannel PrimaryEc(byte mode = 0x02)
    {
    ...
    {
        SetMachineInformation(MechrevoMachine());
        var feature = new TongfangPowerModeFeature(new FakeEcChannel { Available = false });
    ...
    [Fact]
    public async Task IsSupported_ShouldBeFalse_OnNonTongfangMachine()
    {
        SetMachineInformation(new MachineInformation { Vendor = "LENOVO", MachineType = "83DF", Model = "Legion Y9000P IRX9" });
        var feature = new TongfangPowerModeFeature(PrimaryEc());
    ...
    [Fact]
    public async Task IsSupported_ShouldDetectPrimaryLayout()
    {
        SetMachineInformation(MechrevoMachine());
        var feature = new TongfangPowerModeFeature(PrimaryEc(0x03));
    ...
    [Fact]
    public async Task IsSupported_ShouldDetectSecondaryLayout()
    {
        SetMachineInformation(MechrevoMachine());
        var ec = new FakeEcChannel();
        ec.Seed(0xE4, 0x00);

        var feature = new TongfangPowerModeFeature(ec);

        (await feature.IsSupportedAsync()).Should().BeTrue();
        (await feature.GetStateAsync()).Should().Be(PowerModeState.Quiet);
        ResetCompatibilityCache();
    }

    [Theory]
    [InlineData(PowerModeState.Quiet, 0x01)]
    [InlineData(PowerModeState.Balance, 0x02)]
    [InlineData(PowerModeState.Performance, 0x03)]
    public async Task SetState_ShouldWriteAndVerifyReadBack_PrimaryLayout(PowerModeState state, byte expectedValue)
    {
        SetMachineInformation(MechrevoMachine());
        var ec = PrimaryEc();
        ...
        ec.Writes.Should().ContainSingle()
            .Which.Should().Be((0xD7, expectedValue));
    ...
    public async Task SetState_ShouldThrow_WhenWriteFails()
    {
        ...
        await Assert.ThrowsAnyAsync<System.Exception>(() => feature.SetStateAsync(PowerModeState.Quiet));
        ResetCompatibilityCache();
    }

    [Fact]
    public async Task Facade_ShouldUseTongfangBackend_WhenEarlierBackendsUnsupported()
    {
        SetMachineInformation(MechrevoMachine());
        ...
        var facade = new PowerModeFeature(
            ...
            new TongfangPowerModeFeature(PrimaryEc(0x03)),
            new ClevoPowerModeFeature(new FakeEcChannel { Available = false }));
    ...
    private static MachineInformation MechrevoMachine() => new()
    {
        Vendor = "MECHREVO",
        MachineType = "0000",
