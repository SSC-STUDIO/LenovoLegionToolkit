...
### 🔴 H-005: `NetworkAccelerationRuntime.Stop()` 同步方法仍暴露 — 可死锁
...
### 🔴 H-006: `CustomMousePlugin.RunLifecycleTask` 使用 `GetAwaiter().GetResult()` 可能死锁
...
### 🟡 M-008: `ThemeWatcherRuntime.Stop()` 中 `SystemEvents.UserPreferenceChanged` 取消订阅竞态

- **文件**: `Plugins/CustomMouse/ThemeWatcherRuntime.cs:35-60`
- **严重程度**: Medium
- **类别**: 线程安全
- **描述**: `Stop()` 在 `lock (_gate)` 外调用 `SystemEvents.UserPreferenceChanged -= OnUserPreferenceChanged`。如果在 `Start()` 刚设置 `_cts` 但还未订阅事件时调用 `Stop()`，会导致事件未订阅却尝试取消订阅（无异常，但逻辑不清晰）。更关键的是，`Start()` 和 `Stop()` 中的事件订阅/取消订阅不在同一个锁内，可能导致泄漏。
- **建议修复**: 将 `SystemEvents.UserPreferenceChanged +=` 移到 `lock` 内；确保订阅和取消订阅在同一个同步块配对。
- **状态**: 🟡 Investigating
- **发现日期**: 2026-07-06

### 🟡 M-009: `ThemeWatcherRuntime.OnUserPreferenceChanged` 中在 `lock` 内调用 `Timer.Dispose()`

- **文件**: `Plugins/CustomMouse/ThemeWatcherRuntime.cs:83-84`
- **严重程度**: Medium
- **类别**: 线程安全
- **描述**: `_debounceTimer?.Dispose()` 在 `lock (_gate)` 内被调用。`System.Threading.Timer.Dispose()` 会等待所有待执行的回调完成，如果回调本身需要获取 `_gate` 锁，会导致死锁。
- **建议修复**: 在 `lock` 外调用 `Dispose()`；或使用 `Timer.Change(Timeout.Infinite, Timeout.Infinite)` 先停止定时器，再在 `lock` 外 `Dispose`。
- **状态**: 🟡 Investigating
- **发现日期**: 2026-07-06
...
### 🔴 H-007: `PluginHostContext.ResolveType` 中 catch-all 静默吞掉所有异常
...
### 🟡 M-010: SDK `PluginBase` 硬依赖 `LenovoLegionToolkit.Lib` 程序集名称 — ABI 兼容性风险

- **文件**: `SDK/PluginBase.cs:13`, `SDK/LenovoLegionToolkit.Plugins.SDK.csproj:23-26`
- **严重程度**: Medium
- **类别**: 正确性
- **描述**: SDK 的 `PluginBase` 继承自 `LenovoLegionToolkit.Lib.Plugins.PluginBase`，且 .csproj 中引用了 `LenovoLegionToolkit.Lib` 程序集（`<Private>false</Private>`）。虽然 UDT 主程序在 `LegacyPluginContracts.cs` 中以相同命名空间 `LenovoLegionToolkit.Lib.Plugins` 提供了兼容的基类，但这依赖 UDT 永久维护这个 shim。如果未来 UDT 移除 `LegacyPluginContracts.cs`，所有基于该 SDK 编译的插件将全部无法加载。此外，SDK .csproj 中的 `$(HostLibPath)` 需要在构建时正确指向主机程序集，如果路径不正确，SDK 将无法编译。
- **建议修复**: 在 SDK 和主机之间定义独立的 ABI 接口（不依赖具体程序集），使用 `System.Runtime.Loader` 做插件隔离加载；或在文档中明确 ABI 兼容性策略。
- **状态**: 🟡 Investigating
- **发现日期**: 2026-07-06

### 🟡 M-011: `BridgedHostContext` 使用反射访问主机属性 — API 变更无编译时检查

- **文件**: `SDK/PluginHostContext.cs:308-345`
- **严重程度**: Medium
- **类别**: 可维护性
- **描述**: `BridgedHostContext` 通过反射调用主机 `PluginHostContext` 的属性和方法（`TryReadProperty`、`TryInvokeBoolMethod`）。如果主机 API 重命名或删除了 `Mode`、`AllowSystemActions`、`OwnerWindow`、`OpenPluginSettings`、`ShowDialog` 中任意一项，插件在运行时才会失败（静默降级到 `Preview` 模式），而无编译时错误。这增加了主机 API 变更的风险。
...
### 🔴 H-008: 插件无沙箱隔离 — 以主机全信任级别运行

- **文件**: `SDK/PluginHostContext.cs`, `Plugins/**/*.cs`（所有插件）
- **严重程度**: High
- **类别**: 安全性
- **描述**: 插件直接加载到主机 `AppDomain` 中，以与主程序相同的权限级别运行。插件可以：1) 通过 `ProcessRunner` 执行任意进程；2) 通过 `HttpClientManager` 发起网络请求；3) 通过 P/Invoke 调用任意 Win32 API（如果插件代码包含 P/Invoke）；4) 访问文件系统上的任意路径；5) 通过 `Registry` 类读写注册表。没有权限模型、没有代码访问安全（CAS）策略、没有插件间隔离。一个恶意插件可以完全控制主机系统。
- **建议修复**: 将插件加载到独立的 `AppDomain`（.NET Framework）或使用 `AssemblyLoadContext` 隔离（.NET Core）；定义权限接口，插件只能访问授予的权限；考虑使用进程外隔离（子进程）执行不信任的插件。
- **状态**: 🔴 Open
- **发现日期**: 2026-07-06

### 🟡 M-012: 插件依赖检查未递归解析传递依赖

- **文件**: 插件加载逻辑（待定位具体文件）
- **严重程度**: Medium
- **类别**: 正确性
- **描述**: `Dependencies` 属性声明了直接依赖，但加载时没有递归检查依赖链（A→B→C，若 C 缺失则 A 运行时出错）。虽然 `L-002` 已在 Day 1 中标记，但 Day 6 深入审查确认：没有任何代码实现传递依赖检查。
...
### 🟡 M-014: `SettingsManager<T>.Load()` 在异常时静默返回默认设置 — 数据丢失无提示

- **文件**: `Plugins/Shared/SettingsManager.cs:88-93`
- **严重程度**: Medium
- **类别**: 正确性 / 用户体验
- **描述**: `Load()` 在 `catch` 块中记录错误日志后返回 `new T()`（默认设置），但**不通知调用方**发生了错误。如果设置文件损坏（如部分写入、磁盘错误），用户的所有设置静默丢失，且没有任何 UI 提示。用户可能在新会话中发现所有设置被重置但不知道原因。
- **建议修复**: 在异常时抛出自定义异常（如 `SettingsLoadException`）并在 UI 层捕获显示错误；或返回 `LoadResult<T>` 结构包含 `Value`、`IsDefault`、`Error` 字段。
- **状态**: 🟡 Investigating
- **发现日期**: 2026-07-06
...
### 🟡 M-016: 插件 `SettingsManager<T>` 在多线程下可能返回不一致的设置

- **文件**: `Plugins/Shared/SettingsManager.cs:59-94`
- **严重程度**: Medium
- **类别**: 线程安全
- **描述**: `Load()` 和 `Save()` 都使用 `lock (_lock)` 保护，但 `Update()` 方法（第 140-159 行）先调用 `Load()`（获取锁），然后释放锁，再调用 `Save()`（重新获取锁）。在 `Load()` 和 `Save()` 之间，其他线程可能修改了设置文件，导致 `Update()` 基于过期数据写入。
...
- 🔴 Open (High): 8
- 🟡 Investigating (Medium): 17
- 🟢 Open / ⚪ WontFix (Low): 11
- **总计**: 36

## 审查总结

**10 天审查完成** — 共发现 36 个 Bug / 潜在问题。

### 按严重程度分布
- 🔴 High: 8 个（死锁风险、认证绕过、内存泄漏、并发安全）
- 🟡 Medium: 17 个（线程安全、数据完整性、性能、兼容性）
- 🟢 Low: 11 个（代码质量、可维护性）
...
### 优先修复建议（Top 5）
1. **H-003/H-005/H-007** (Stop() 死锁 + OnShutdown 死锁 + ResolveType 静默异常) — 稳定性关键
2. **H-006** (RunLifecycleTask GetAwaiter().GetResult() 死锁) — UI 响应关键
3. **H-008/H-009** (插件无沙箱 + 无进程隔离) — 安全关键
4. **M-016** (SettingsManager Update TOCTOU) — 数据完整性关键
5. **M-001/M-014** (设置无版本兼容 + Load 静默数据丢失) — 用户体验关键
