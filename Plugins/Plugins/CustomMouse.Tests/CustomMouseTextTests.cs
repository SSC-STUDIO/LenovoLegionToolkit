...
public class CustomMouseTextTests
{
    [Fact]
    public void TextClass_HasNoHardcodedChinese()
    {
        var type = typeof(CustomMouseText);
    ...
    {
        var assembly = Assembly.Load("LenovoLegionToolkit.Plugins.CustomMouse");
        var resourceManager = new ResourceManager(
            "LenovoLegionToolkit.Plugins.CustomMouse.Resources.Resource",
            assembly);

        var textType = typeof(CustomMouseText);
        var properties = textType.GetProperties(BindingFlags.Public | BindingFlags.Static)
            .Where(p => p.PropertyType == typeof(string))
            .ToList();

        Assert.True(properties.Count > 0, "No string properties found in CustomMouseText");
