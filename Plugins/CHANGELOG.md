...
- **Workflow Runtime Maintenance / 工作流运行时维护**: Updated the plugin CI workflow to current official `actions/*` releases, switched ZIP packaging uploads to a dedicated `Build/artifacts` directory, and replaced the fragile cross-step plugin selection output with an explicit job environment variable so build/package steps no longer drop the selected plugin list on Windows runners / 将插件 CI 工作流升级到当前官方 `actions/*` 版本，把 ZIP 打包产物统一输出到 `Build/artifacts` 目录，并将脆弱的跨步骤插件列表输出改为显式作业环境变量，避免在 Windows runner 上丢失选中的插件列表，进而导致构建/打包步骤空跑
