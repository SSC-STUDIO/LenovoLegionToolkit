...
namespace UniversalDeviceToolkit.Tests.FlaUI
{
    ...
    /// Launches the UDT application under test and provides helpers for UI interaction and WinRT OCR verification.
    /// </summary>
    public abstract class FlaUiTestBase : IAsyncLifetime
    {
        ...
        public static bool IsCiEnvironment()
        {
            ...
            // Also check for headless session (e.g., SSH without desktop)
            var sessionName = Environment.GetEnvironmentVariable("SESSIONNAME");
            if (sessionName != null && sessionName.Equals("Services", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        ...
        {
            // Skip in CI environments — no desktop session available for UI automation
            if (IsCiEnvironment())
            {
                throw new SkipException("FlaUI tests are skipped in CI environments (no desktop session).");
            }
            ...
            catch (System.ComponentModel.Win32Exception ex)
            {
                ...
                if (isElevationError)
                {
                    throw new SkipException(
                        ...
                        $"Please re-run the test runner as Administrator. Original error: {ex.Message}");
                }

                throw new SkipException(
                    $"Failed to launch UDT application: {ex.Message}");
            }
