...
/// <summary>
/// Generic, read-only Windows machine adapter. Vendor-specific writes stay in
/// the existing verified feature backends and are never performed here.
/// </summary>
public sealed class WindowsDeviceAdapter : IDeviceAdapter
{
        ...
        IReadOnlyDictionary<string, string?> video)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Value(video, "Name")))
            readings.Add(new SensorReading("GPU", "GPU", 1, Value(video, "Name")));
