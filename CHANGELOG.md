...
- 插件侧栏导航项补充确定性的 UIA Invoke 路径，使本地功能页烟测不再依赖物理点击是否命中动态导航项 / Plugin sidebar navigation items now expose a deterministic UIA Invoke path so local feature-page smoke no longer depends on physical clicks landing on dynamic navigation items
- 插件设置窗口现在显示插件声明或随包提供的图标，并在缺失或加载失败时回退到默认图标 / Plugin settings windows now show declared or packaged plugin icons and fall back to the default icon when they are missing or fail to load
- 系统优化页右上角操作与批量操作已移入工具栏，并补充稳定的 AutomationId，便于 UI 冒烟和辅助功能定位 / System Optimization top-right and bulk actions now live in the toolbar with stable AutomationIds for UI smoke and accessibility targeting
- `MainAppPluginUi.Smoke` 为 `network-acceleration` 的 Configure 路径新增鼠标点击回退，减少 UIA Invoke 偶发失效导致的误报 / `MainAppPluginUi.Smoke` now adds a mouse-click fallback for the `network-acceleration` Configure path, reducing false failures when UIA Invoke intermittently misses
- 插件扩展页卡片操作升级为带图标的主程序按钮，并补充侧栏功能页能力标签，提升插件安装、配置、打开与卸载动作的可读性 / Plugin Extensions cards now use host-style icon buttons and show a sidebar feature-page capability tag, making install, configure, open, and uninstall actions easier to scan
- `MainAppPluginUi.Smoke` 新增 `--disable-animations` 和 `--animation-speed-ms` 参数，支持禁用动画或自定义动画速度以加快非观察模式下的测试执行 / `MainAppPluginUi.Smoke` now supports `--disable-animations` and `--animation-speed-ms` parameters to disable animations or customize animation speed for faster test execution in non-watch mode
- `MainAppPluginUi.Smoke` 弹窗检测现已精确分类为 `WpfUiMessageBox`、`SystemWindowsMessageBox` 和 `NotificationPopup` 三种类型，并在日志中输出具体类型和消除方式 / `MainAppPluginUi.Smoke` popup detection now classifies popups into `WpfUiMessageBox`, `SystemWindowsMessageBox`, and `NotificationPopup` types with detailed logging of type and dismissal method
- `MainAppPluginUi.Smoke` 新增 `ResolveLiveWindowAndDismissPopups` 和 `WaitForAnimationsAndResolveWindow` 封装方法，减少重复代码模式 / `MainAppPluginUi.Smoke` adds `ResolveLiveWindowAndDismissPopups` and `WaitForAnimationsAndResolveWindow` helper methods to reduce repetitive code patterns
- `MainAppPluginUi.Smoke` 在测试结束时输出弹窗消除摘要，便于复盘测试过程中遇到的弹窗 / `MainAppPluginUi.Smoke` now outputs a dismissed popups summary at test completion for easier review of popups encountered during testing
- `MainAppPluginUi.Smoke` 新增 MessageBox 和 NotificationWindow 弹窗检测与自动消除机制，避免弹窗阻塞测试流程 / `MainAppPluginUi.Smoke` now detects and automatically dismisses MessageBox and NotificationWindow popups to prevent them from blocking the test flow
- `MainAppPluginUi.Smoke` 增强动画兼容性：在窗口关闭、页面导航、按钮点击等关键操作后增加动画等待时间，确保 UI 元素在动画完成后才进行后续操作 / `MainAppPluginUi.Smoke` now includes animation-aware waiting after window closes, page navigation, and button clicks to ensure UI elements are fully ready before subsequent operations
- `MainAppPluginUi.Smoke` 改进窗口关闭等待逻辑，在显式关闭按钮点击后增加动画完成等待，避免因窗口关闭动画未完成导致的检测失败 / `MainAppPluginUi.Smoke` improved window close wait logic with animation completion grace period after explicit close button clicks to prevent detection failures caused by incomplete close animations
- `MainAppPluginUi.Smoke` 现在会优先通过应用内 IPC 直接导出主窗口与设置窗口的视觉树，并在系统优化页截图里先捕获干净状态再执行动作，使深浅色 UI 评审不再被透明窗口截屏失真或运行时错误提示条遮挡 / `MainAppPluginUi.Smoke` now prefers in-app IPC visual exports for main and settings windows and captures a clean pre-action state for System Optimization pages, preventing Dark/Light UI reviews from being distorted by transparent-window screenshot failures or obscured by runtime error banners
- `MainAppPluginUi.Smoke` 新增 `shell-local`、`combo-local` 和 `--theme system|light|dark` 预设，可在干净沙箱里对本地安装的 `shell-integration` 与 `custom-mouse + shell-integration` 组合做真实 Dark/Light 冒烟，并把主题与场景写入截图索引 / `MainAppPluginUi.Smoke` now supports `shell-local`, `combo-local`, and `--theme system|light|dark` presets so local `shell-integration` and `custom-mouse + shell-integration` combinations can be smoke-tested in a clean sandbox under real Dark/Light themes with the selected scenario written into the screenshot manifest
- `MainAppPluginUi.Smoke` 现在会在 `shell-integration` 设置页内继续验证 `Open Style Settings`，把样式编辑弹窗纳入真实本地安装后的功能链路 / `MainAppPluginUi.Smoke` now continues from the `shell-integration` settings page into `Open Style Settings`, bringing the style editor dialog into the real post-install functional smoke path
- 插件设置宿主壳完成一轮纯 UI 打磨：放大并简化插件设置窗口，减少双层滚动和过重的嵌套容器 / The plugin settings host shell received a UI-only polish pass that enlarges and simplifies the plugin settings window to reduce nested containers and double-scroll behavior
- `Menu Style Settings` 和插件侧 `shell-integration` / `custom-mouse` 设置页统一收敛到更接近宿主的卡片层级、间距和深浅色表现，改善大尺寸窗口下的固定盒子与灰上加灰观感 / `Menu Style Settings` plus the plugin-side `shell-integration` and `custom-mouse` settings pages were tightened to match the host shell more closely in card hierarchy, spacing, and Dark/Light presentation, reducing the fixed-box layout and over-layered gray-on-gray appearance in large windows
...
- 加固 `MainAppPluginUi.Smoke` 的真实安装验证：为 Plugin Extensions footer 导航增加键盘回退，并把“已安装”判定同步到当前插件卡片按钮文本状态，使真实在线安装与本地 ZIP 导入的 UI smoke 在当前宿主上更稳定 / Hardened `MainAppPluginUi.Smoke` real-install verification by adding a keyboard fallback for the Plugin Extensions footer navigation and aligning installed-state detection with the current plugin card button text so real online installs and local ZIP imports are more reliable on the current host
- 为 `MainAppPluginUi.Smoke` 新增按运行会话归档的页面截图与截图索引，并扩展 `--screenshots`、`--screenshot-dir`、`--keep-artifacts`、`--list-plugins`、`--help` 等 CLI 参数，方便逐页检查 UI 文案与布局 / Added per-run page screenshot capture with an indexed screenshot manifest to `MainAppPluginUi.Smoke`, and expanded its CLI with `--screenshots`, `--screenshot-dir`, `--keep-artifacts`, `--list-plugins`, and `--help` for page-by-page UI text and layout review
- `MainAppPluginUi.Smoke` 现在会把“插件页面加载失败”文案识别为真正失败，不再把带异常消息的空页面误判为通过 / `MainAppPluginUi.Smoke` now treats visible “failed to load plugin page” content as a real failure instead of falsely passing a broken plugin page that only rendered an exception message
- `MainAppPluginUi.Smoke` 的本地插件功能验证现在会在插件市场控件刷新后重新解析 live UI 元素，并在 `network-acceleration` 的卡片入口偶发失效时回退到 sidebar/configure 路径继续执行真实功能校验，减少安装后立刻验证时的竞态误判 / `MainAppPluginUi.Smoke` local plugin functional validation now re-resolves live UI elements after marketplace refreshes and falls back to the sidebar/configure routes when `network-acceleration` card entry checks intermittently miss, reducing race-driven false failures right after install
- 新增专用的 `Main App UI Smoke` workflow 与 PowerShell runner 包装脚本，可把真实 UI smoke 放到独立的交互式 Windows runner 上执行并自动回传截图与日志，避免打扰本地开发桌面 / Added a dedicated `Main App UI Smoke` workflow and PowerShell runner wrapper so real UI smoke can run on a separate interactive Windows runner and automatically upload screenshots and logs without disturbing the local development desktop
- `MainAppPluginUi.Smoke` 新增可观看的 `--watch` 观察模式与步进/收尾停留参数，使真实桌面 smoke 可以按肉眼可跟踪的节奏播放，并保留成功或失败现场供直接查看 / Added a watchable `--watch` mode plus step-delay and end-of-run hold controls to `MainAppPluginUi.Smoke`, so real desktop smoke can play at a human-observable pace and keep the success or failure state on screen for direct inspection
- `MainAppPluginUi.Smoke` 现在会在截图目录中额外生成 `storyboard.html`，可直接按顺序回放每一步截图，便于离线复核真实桌面 smoke 的页面切换过程 / `MainAppPluginUi.Smoke` now emits a `storyboard.html` player alongside the screenshots so the real desktop smoke flow can be reviewed offline step by step in order
- 插件市场在线条目现在会在“已获取商店列表但更新检查超时”时继续显示，不再因为后续更新检查失败把整个在线插件页清空，这修复了 `vive-tool` 和 `network-acceleration` 最新线上包在真实 smoke 中被误判为“市场不可用”的问题 / The plugin marketplace now keeps showing fetched online entries even when the follow-up update check times out, instead of clearing the whole online list after a secondary failure. This fixes real smoke runs falsely treating the latest `vive-tool` and `network-acceleration` online packages as missing from the marketplace
- 在线插件下载现在优先尝试稳定的 browser download URL，并对瞬时传输失败执行真正的多次重试，而不是单次失败即终止，这改善了最新 GitHub Release 插件在真实安装 smoke 下的下载成功率 / Online plugin downloads now prefer the stable browser download URL and perform real multi-attempt retries for transient transport failures instead of stopping after a single failed attempt, improving real install-smoke reliability for the latest GitHub Release plugins
- 插件仓库下载超时已放宽到更接近当前真实网络条件，避免在这台机上把仍在传输中的 GitHub Release ZIP 过早取消 / Plugin repository download timeouts are now tuned closer to the current real network conditions on this workstation, so GitHub Release ZIP transfers are less likely to be canceled while they are still making progress
- `MainAppPluginUi.Smoke` 的安装成功判定现在会在 UIA 按钮状态之外回读沙箱 `settings.json` 和插件目录，避免插件已实际安装但市场卡片尚未刷新时被误判为超时失败 / `MainAppPluginUi.Smoke` now falls back to the sandbox `settings.json` and plugin directory in addition to UIA button state when deciding whether an install succeeded, preventing false timeout failures after a real install completes before the marketplace card refreshes
- 插件市场在线安装现在会串行执行，`MainAppPluginUi.Smoke` 也同步放宽了真实在线安装等待窗口，避免下载重试期间被并发点击串出第二个安装任务，或在第二次下载尝试完成前被 smoke 过早判定为失败 / Online marketplace installs now run serially, and `MainAppPluginUi.Smoke` now allows a longer real-install wait window so retries are not interrupted by a second concurrent install trigger or marked failed before a later download attempt can finish
- 在线插件下载在 Windows 上对 GitHub browser release URL 新增了原生 `curl.exe` 兜底路径，避免 `HttpClient` 连续中断后真实安装 smoke 和终端用户都被同一批网络重置卡死 / Online plugin downloads on Windows now add a native `curl.exe` fallback for GitHub browser release URLs so repeated `HttpClient` transport resets no longer block both real install smoke and end users on the same network conditions
- `MainAppPluginUi.Smoke` 刷新插件页后的“页面就绪”判定已放宽到页面级信号，不再硬等按钮先出现，避免在线列表已经刷出但按钮模板稍后到达时被误报为插件页加载失败 / `MainAppPluginUi.Smoke` now treats marketplace readiness after refresh using page-level signals instead of waiting only for action buttons, avoiding false failures when the online list is already visible but button templates arrive slightly later
- `MainAppPluginUi.Smoke` 现在会在插件卡片动作按钮已可交互时继续执行，即使插件市场后台刷新仍显示 `Loading plugins...`，避免本地 ZIP 导入后的真实功能验证被后台商店加载状态误拦截 / `MainAppPluginUi.Smoke` now proceeds once a plugin card exposes actionable buttons even if the marketplace background refresh still shows `Loading plugins...`, preventing real local ZIP functional checks from being blocked by residual store-loading state
- `MainAppPluginUi.Smoke` 对 `custom-mouse` 旧版独立页遗留的 reentry 清理已降级为 best-effort，不再因为设置窗口关闭后的宿主级重入细节阻断真实的本地安装与功能验证 / `MainAppPluginUi.Smoke` now treats the legacy `custom-mouse` reentry cleanup path as best-effort so host-side settings-window reentry quirks no longer block real local-install and functional validation

### Fixed / 修复
- 首次启动或新沙箱缺少配置文件时不再把预期的默认配置加载路径写成完整异常栈，减少日志误报 / Missing configuration files on first launch or in new sandboxes no longer produce full exception stacks when defaults are used, reducing misleading log noise
- 更新设置页在主程序通过 `--disable-update-checker` 启动时不再显示空白，而是明确说明本次会话的更新检查和相关设置已被启动参数禁用 / The Update settings page no longer appears blank when the app is started with `--disable-update-checker`; it now explains that update checks and related settings are disabled for this session by the startup argument
- 不支持 RGB 键盘的设备在退出时不再把预期的所有权跳过写成完整异常栈，减少日志中的误导性错误信息 / Devices without RGB keyboard support no longer log expected ownership skips as full exception stacks during shutdown, reducing misleading error noise in logs
- 修复驱动下载扫描失败路径可能跨线程显示提示导致日志写入异常的问题，修正驱动下载来源选项在深色模式下仍显示深色文字的配色，并避免日志系统把包含花括号的普通消息误解析成格式化模板 / Fixed Driver Download scan failure handling that could log a cross-thread snackbar exception, corrected source-option text colors that stayed dark in Dark theme, and prevented the logger from parsing plain messages containing braces as format templates
- 启动语言选择窗口现在会在主窗口创建前按 `settings.json` 的 Light/Dark/System 配置套用主题，并按已保存语言或系统语言预选当前语言 / The startup language selector now applies the configured Light/Dark/System theme before the main window is created and preselects the saved or system language
- 修复系统优化页中清理命令无法执行 `del`/`rd` 内建命令的问题，并将“高性能电源计划”动作改为真正启用高性能方案 / Fixed System Optimization cleanup actions failing to execute `del`/`rd` shell built-ins and corrected the high-performance power-plan action to activate the actual high-performance scheme
- 插件依赖解析现在按请求程序集限定插件目录，避免多个插件携带同名 sidecar DLL 时串用其他插件目录中的依赖 / Plugin dependency resolution is now scoped to the requesting assembly so plugins with same-named sidecar DLLs do not borrow dependencies from another plugin directory
- 插件依赖加载现在允许强命名且链路可信的微软 sidecar DLL 使用已过期签名，避免 NuGet 随包依赖因证书过期被误拦截 / Plugin dependency loading now allows strong-named Microsoft sidecar DLLs with trusted chains and expired signatures, preventing packaged NuGet dependencies from being blocked only because the signing certificate expired
- 修复插件功能页懒加载时无法从插件目录解析 sidecar 依赖的问题，避免已随包安装的 `Microsoft.Extensions.Logging.Abstractions` 等 DLL 仍被误报缺失 / Fixed plugin feature-page lazy loading failing to resolve sidecar dependencies from the plugin directory, preventing packaged DLLs such as `Microsoft.Extensions.Logging.Abstractions` from being reported as missing
- 修复插件功能页在创建页面期间响应插件资源文化刷新而反复重建的问题，避免 ViVeTool 页面初始化循环卡住 / Fixed plugin feature pages rebuilding repeatedly when plugin resource-culture refreshes fire during page creation, preventing the ViVeTool page from getting stuck in an initialization loop
- 修复插件功能页在不同加载上下文中创建失败的问题，并展开内部异常以显示真实错误原因 / Fixed plugin feature pages failing to create across different load contexts and unwrapped inner errors so the real cause is shown
- 修复 `MainAppPluginUi.Smoke` 仍共用正式主程序单实例锁的问题，使隔离沙箱 smoke 不会因为桌面上已有 LLT 实例或残留单实例对象而在启动阶段自退 / Fixed `MainAppPluginUi.Smoke` sharing the production single-instance lock so isolated sandbox smoke runs no longer self-exit at startup when another LLT instance or stale single-instance object is present on the desktop
- 修复隔离沙箱 smoke 使用独立单实例键后，主程序退出路径仍无条件释放 mutex 导致的 `ReleaseMutex` 异常，避免真实实机验证结束时额外写入误导性崩溃日志 / Fixed the app shutdown path unconditionally releasing the single-instance mutex after sandboxed smoke runs switched to isolated instance keys, preventing misleading `ReleaseMutex` exceptions from being logged at the end of real machine validation
- 修复插件市场在在线列表已加载但更新元数据随后到达时不会再次刷新卡片的问题，避免单个插件的更新徽标、版本号和 changelog 状态滞后于底部批量更新提示 / Fixed the plugin marketplace cards not refreshing after online update metadata arrived so per-plugin update badges, versions, and changelog state no longer lag behind the bulk-update footer
- 清理 `Plugin Extensions` 页面已失效的详情面板动画收尾路径，避免卸载或永久删除插件时继续往不存在的 storyboard 叠加 `Completed` 事件处理器 / Removed the stale detail-panel animation cleanup path from `Plugin Extensions` so uninstalling or permanently deleting plugins no longer stacks `Completed` handlers onto a storyboard that no longer exists
