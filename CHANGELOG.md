...
- **[Packaging post–Phase 3]** Restore winget package identity `SSC-STUDIO.LenovoLegionToolkit` and historical scoop/winget installer URLs (`LenovoLegionToolkit_v*_Setup.exe`); fix Release `LEGACY_SETUP_ASSET`, `udt-cli.dll` smoke check, and winget validate path; ship `llt.exe`/`llt.dll` as one-release shims of `udt-cli`; dual-stage plugin SDK/Shared under both UDT and legacy filenames / **[打包 Phase 3 后修复]** 恢复 winget 包 ID 与历史 scoop/winget 安装包 URL；修正 Release 兼容别名、CLI smoke 与 winget 校验路径；发布期提供 `llt`→`udt-cli` 兼容拷贝；插件 SDK/Shared 双文件名落盘
