...
`llt-plugin.cmd` is a compatibility alias. Host baseline is **v5.0.0** (`Plugins/HostBaseline/host-release.json`); host binaries are downloaded into the ignored `.host/` cache.
