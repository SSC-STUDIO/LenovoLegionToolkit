...
`llt-plugin.cmd` is a compatibility alias. The published host baseline is **v5.0.2** (`Plugins/HostBaseline/host-release.json`); host binaries are downloaded into the ignored `.host/` cache.
