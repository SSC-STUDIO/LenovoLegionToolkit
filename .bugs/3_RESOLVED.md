...
- **Symptom**: Dispose(bool) used taskToWait?.Wait(TimeSpan.FromSeconds(5)) — sync-over-async anti-pattern — blocking the disposing thread on a threadpool task. Additionally, if (_disposed) return was a non-atomic check-then-set allowing concurrent double-dispose of CTS/refresh task.
