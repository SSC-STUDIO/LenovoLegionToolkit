...
### [2026-07-09] Non-blocking Dispose parity + atomic dispose guard for periodic monitor services (BUG-2026-07-09-008)
- **Symptom**: BatteryDischargeRateMonitorService.Dispose(bool) called taskToWait?.Wait(TimeSpan.FromSeconds(5)) — synchronous blocking on a threadpool task — while its StopAsync() counterpart already used non-blocking await taskToWait.WaitAsync(...). The Dispose path also used a non-atomic check-then-set (if (_disposed) return) allowing two concurrent Dispose calls to both pass the guard and double-dispose the CTS/refresh task.
- **Root Cause**: (1) Teardown parity broken — Dispose blocked the disposing thread while StopAsync did not. (2) _disposed was a plain bool, not an interlocked CAS, so the guard was non-atomic.
- **Enforced Rule** (supplements the BUG-005 Pillar A.1 rule): For any IDisposable monitor/service that owns a CancellationTokenSource + background Task, Dispose(bool) MUST:(a) guard entry with Interlocked.CompareExchange(ref _disposed, 1, 0) — never a non-atomic check-then-set, even if volatile.(b) MINOR a bounded Wait(TimeSpan) is acceptable for the timed-dispose fallback ONLY when the awaited task runs ConfigureAwait(false) on the threadpool (no Dispatcher hop), but MUST first short-circuit via 	ask.IsCompletedSuccessfully to avoid blocking in the common completed case, matching the AIController precedent. (c) On Wait timeout, leave the task running fire-and-forget and attach a ContinueWith(OnlyOnFaulted | ExecuteSynchronously) observation hook so a late fault never escalates as an unobserved-task exception. (d) Wrap the wait in try/AggregateException so a faulted task does not escape Dispose. Dispose parity with StopAsync must be maintained — if StopAsync uses non-blocking WaitAsync, Dispose must not regress to blocking.
- **OS / .NET Version**: Windows 11 24H2, .NET 10, WPF